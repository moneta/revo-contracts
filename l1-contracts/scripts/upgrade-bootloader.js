...
const setNewVersionUpgradeFunctionData = async (
  ...
  const functionData = stm.interface.encodeFunctionData("setNewVersionUpgrade", [
    // DiamondCutData
    [
      [], // facetCuts
...
const _setProtocolVersionDeadline = async (version, timestamp) => {
  const stm = await hre.ethers.getContractAt("ChainTypeManager", config.stmAddress);
  const result = stm.interface.encodeFunctionData("setProtocolVersionDeadline", [version, timestamp]);
  return result;
};
...
async function main() {
  ...
  switch (STEP) {
    ...
    case 2:
      ...
      break;
    case 3:
      callData = await setNewVersionUpgradeFunctionData(
        bootloaderCodHash,
        upgradeTimestamp,
        deadline,
        oldProtocolVersion,
